...
it('호출 시 initialValue 인자를 지정하지 않는 경우 isModalOpened 상태가 false로 설정된다.', () => {
  // 리액트 훅은 반드시 리액트 컴포넌트 내에서만 호출되어야 정상적으로 실행된다 -> renderHook()을 사용하여 테스트
  // result: 훅을 호출해 얻은 결과 값을 반환, result.current로 최신 상태를 추적할 수 있다
  // rerender: 훅을 원하는 인자와 함께 새로 호출해서 상태를 갱신할 수 있다
